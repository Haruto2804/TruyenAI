...
import { Plus, List, Edit, BookOpen, Clock, Layers, ArrowRight } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-0.5">
            Quản trị nội dung, nhân vật, thuật ngữ và các chương truyện
          </p>
        </div>
              ...
              <div className="pt-2 border-t border-white/5 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Link 
                    ...
                    className="flex-1 min-h-[42px] px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-200 border border-white/10 text-xs font-semibold flex items-center justify-center gap-1.5 active:scale-95 transition-all"
                  >
                    <Edit className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Sửa truyện</span>
                  </Link>

                  <form action={deleteStory} className="shrink-0">
                    <input type="hidden" name="id" value={story.id} />
                    <DeleteButton 
                      message="Xóa truyện sẽ xóa TẤT CẢ các chương, nhân vật và chú giải. Bạn có chắc chắn không?" 
                      className="min-h-[42px] px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/20 text-xs font-semibold flex items-center justify-center gap-1.5 active:scale-95 transition-all"
                    />
                  </form>
                </div>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-800/60 text-slate-400 uppercase text-xs">
              <tr>
                <th className="px-6 py-4 font-semibold">Tên Truyện</th>
                <th className="px-6 py-4 font-semibold">Thể Loại</th>
                <th className="px-6 py-4 font-semibold">Số Chương</th>
